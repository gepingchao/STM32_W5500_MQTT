...
int MutexLock(Mutex* mutex)
{
	...
	//return xQueueTakeMutexRecursive(mutex->sem, portMAX_DELAY); 
}

int MutexUnlock(Mutex* mutex)
{
	return xSemaphoreGive(mutex->sem);
	//return xQueueGiveMutexRecursive(mutex->sem);
}
...
int FreeRTOS_read(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	do
	{
		...
		rc = recv_timeout(n->my_socket, buffer + recvLen,  len - recvLen,xTicksToWait);
...
int FreeRTOS_write(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	do
	{
		...
		//ctlsocket(n->my_socket,CS_SET_IOMODE,(void*)SOCK_IO_NONBLOCK);
...
int NetworkConnect(Network* n, char* addr, int port)	//char addr[4] = {192,168,1,214};
{
	...
	socket_status = getSn_SR(n->my_socket);
	if(SOCK_CLOSED == socket_status)
		{
			if(socket(n->my_socket,Sn_MR_TCP,0,Sn_MR_ND) != (n->my_socket))
				{
					printf("socket error\r\n");
				}
		}
	...
	if(retVal = connect(n->my_socket,addr,port))
		{
		}
		else
		{
